...
class WorkspaceView extends Component {
        ...
        WorkspaceService.getPermissions()
            .then(permission => {
                if (permission.status === 200) {
                    this.bootstrapWorkspace(permission.data.data, user);
                }
            })
            .catch(eres => {
                debugger
    ...
    initiateWorkspace(e) {
        ...
        WorkspaceService.readyWorkspace(this.state.newWorkspace.name, sub)
            .then(workspace => {
                debugger
                if (workspace.data.status) {
                    ...
                    this.createBillingAccount(user, workspace.data.data);
                } else {
                    toastr.error('Failed to create Workspace', 'Something went wrong. Please try again later.');
    ...
    getBillingAccount (user) {
        SubscriptionService.getAccount(user)
            .then(res => {
                if (res.data.IsSuccess) {
    ...
    bootstrapWorkspace (workspace, user) {
        this.setState(s => ({...s, loading: true}));
        localStorage.setItem('scopes', workspace);
        const authr = this.props.uihelper.authredirect;
        if (authr) {
            // const user = this.props.user.user;
            // const exp = UIHelperService.parseJWT(user.jwtToken).exp;
            // document.cookie = "scopes=" + workspace + ";expires=" + exp + ";path=/";
            // document.cookie = "token=" + user.jwtToken + ";expires=" + exp + ";path=/";
            window.location.replace(authr);
        } else {
